...
for (const tds of gemini15Data) {
    originalMetricsRaw.push({
        gptzero: -1,
        zerogpt: tds.original_text_metrics.detection_scores.zerogpt,
        roberta: tds.original_text_metrics.detection_scores.roberta,
    });
}
...
for (let i = 0; i < originalTextsRaw.length; i++) {
    ...
    for (const model of models) {
        ...
        const summarizedText: Text = {
            ...
            detectionScores: {
                gptzero: -1,
                zerogpt: modelDataRaw.summarized_text_metrics.detection_scores.zerogpt,
                roberta: modelDataRaw.summarized_text_metrics.detection_scores.roberta,
            },
        ...
        for (const iteration of modelDataRaw.iterations) {
            ...
            iterations.push({
                ...
                detectionScores: {
                    gptzero: -1,
                    zerogpt: iteration.detection_scores.zerogpt,
                    roberta: iteration.detection_scores.roberta,
                },
