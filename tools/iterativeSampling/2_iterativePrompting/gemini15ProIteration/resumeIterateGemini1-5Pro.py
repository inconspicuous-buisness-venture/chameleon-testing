...
def get_highest_iteration_number(data):
    """Determine the highest completed iteration in the data."""
    max_iter = 0
    for item in data:
        if "iterations" in item and item["iterations"]:
            item_max = max(iter_data["iteration_number"] for iter_data in item["iterations"])
            max_iter = max(max_iter, item_max)
    return max_iter
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process text through iterative Gemini prompts')
    parser.add_argument('--start-iteration', type=int, default=1, 
                        help='Iteration number to start from (default: 1)')
    args = parser.parse_args()
    
    # Initialize API keys
    api_keys = [
        "AIzaSyC78w7ZBMaguiz9neYJmEuQjh9eXYe6bC8",  # Original key
        "AIzaSyBvXnbbT5v05GZ9iXLde1y1E66OpOGZdhc",  # New key 2
        "AIzaSyAWz3uTVRd8WQziONOpYc1sXajagBShYO8",  # New key 3
        "AIzaSyDAz5GUqDq_pX8c0Bela_cy8T8fm7WbEA0",  # New key 4
        "AIzaSyB8RZbKkL0vuRCNBDk8NYcpOAkJCjTTKhY",  # New key 5
        "AIzaSyDfJ5MzwAIDYrYCbhzgvNwhSQzTWAKRasI",  # New key 6
        "AIzaSyBNKM03kukcWRMaqGJKGE8w74BPKSpt-DY",  # New key 7
    ]
    ...
    # Get initial input
    initial_input_json = input("Enter path to input JSON file with existing iterations: ")
    output_dir = input("Enter directory for output files: ")
    os.makedirs(output_dir, exist_ok=True)
    
    # Setup logging
    log_file = os.path.join(output_dir, f"log_resumed_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    # Read existing data with iterations
    data = read_json_data(initial_input_json)
    if not data:
        logger.error("Failed to load input JSON data.")
        return
    
    # Identify the highest iteration already completed
    highest_completed_iteration = get_highest_iteration_number(data)
    
    # Decide where to start
    start_iteration = max(args.start_iteration, highest_completed_iteration + 1)
    logger.info(f"Highest completed iteration in data: {highest_completed_iteration}")
    logger.info(f"Will resume from iteration {start_iteration}")
    
    # Run iterations from the starting point
    total_iterations = 10  # Total iterations to complete
    output_json = os.path.join(output_dir, "iterations_output_complete.json")
    ...
    for iteration in range(start_iteration, total_iterations + 1):
        ...
        for key, info in usage_report.items():
            logger.info(f"  {key}: {info['requests']} requests (exhausted: {info['exhausted']})")
