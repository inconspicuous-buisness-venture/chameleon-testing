...
def rewrite_text(text, client, max_retries=3):
    """Use ChatGPT-4o API to rewrite the text with retry functionality."""
    prompt = f"""
Rewrite the following text in a new way while keeping the meaning, tone, and key details intact. 
Restructure sentences, and use varied phrasing as necessary to make it feel naturally rewritten 
while keeping it recognizable as the same content. Ensure clarity and coherence. 
DO NOT ADD TITLES, LABELS, HEADINGS, OR WORDS OTHER THAN REWRITING THE TEXT. 
This rewrite should be around the similar length to the original given text which is given below:
    ...
    while retry_count <= max_retries:
        try:
            response = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant that rewrites text while preserving meaning."},
